...
import { getImageryGallery } from "@/lib/api/queries";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getImageryGallery(modelId, paintId, paintDescription);
    return NextResponse.json(data);
  } catch {
    return NextResponse.json({ modelId, source: "imagin", slides: [] });
  }
}
